...
export default function FloorView() {
  ...
  // Real live data for bottom strip
  ...
  const { data: customerEntries } = useCustomerEntries(today(), today());

  const guestCount = (guestVisits?.length ?? 0) + (customerEntries?.length ?? 0);
  const dancersActive = (activeDancers ?? []).filter((d) => d.live_status !== "inactive").length;
  const roomsOccupied = (activeSessions ?? []).length;
  const roomsOvertime = (activeSessions ?? []).filter(
    (s) => Date.now() - new Date(s.entry_time).getTime() > 15 * 60 * 1000,
  ).length;
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-40 bg-card/95 backdrop-blur-md border-t border-border px-6 py-2.5 flex items-center justify-center gap-6 md:gap-10 text-sm">
        <span className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground">Guests On Floor</span>
        </span>
        <span className="text-border">|</span>
        <span>
          <strong>{dancersActive}</strong>{" "}
          <span className="text-muted-foreground">Dancers Active</span>
        </span>
        <span className="text-border hidden sm:inline">|</span>
        <span className="hidden sm:inline">
          <strong>{roomsOccupied}</strong>{" "}
          <span className="text-muted-foreground">Rooms Occupied</span>
        </span>
        <span className="text-border hidden sm:inline">|</span>
        <span className="hidden sm:flex items-center gap-1.5">
          <span className={`w-2 h-2 rounded-full ${roomsOvertime > 0 ? "bg-destructive animate-pulse-glow" : "bg-muted-foreground"}`} />
          <strong>{roomsOvertime}</strong>{" "}
          <span className="text-muted-foreground">Room Overtime</span>
        </span>
      </footer>
    </div>
  );
